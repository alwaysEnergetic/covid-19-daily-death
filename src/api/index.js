  ...
  try {
    const statesData = await axios.get(API_STATES).then((response) => {
      ...
      if (state === undefined) {
        ...
        const filteredState = mappedStates.filter(
          (data) => data.fullName === state
        );
        const {
          stateAbbrev,
          fullName,
...
const fetchStatesDaily = async (state) => {
  try {
    const dailyStatesData = await axios
      .get(API_STATES_DAILY)
      .then((response) => {
        const filteredState = response.data.filter(
...
export { fetchUS, fetchDailyUS, fetchStates, fetchStatesDaily };
